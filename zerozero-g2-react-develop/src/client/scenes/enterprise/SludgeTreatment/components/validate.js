...
const validate = values => {
    ...
    if(!v.items || v.items.length === 0) {
        ...
        const itemOther = v.items.find(item => item.code === 'D-Other');
        if(itemOther && !v.itemOther) {
            errors.itemOther = '必填';
        }
    }

    if(!v.packTypes || v.packTypes.length === 0) {
        errors.packTypes = '必填';
    } else {
        const packOther = v.packTypes.find(item => item.code === 'SPK99');
        if(packOther && !v.packOther) {
            errors.packOther = '必填';
        }
    }
