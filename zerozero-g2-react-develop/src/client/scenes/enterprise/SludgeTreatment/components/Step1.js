...
class ReservationStep1 extends Component {
    ...
    constructor(props) {
        ...
        this.packTypeMultiSelectOnChange = this.packTypeMultiSelectOnChange.bind(this);

        this.state = {
            renderItemOtherTextField: false,
            renderPackOtherTextField: false
        }
    }
    ...
    packTypeMultiSelectOnChange(e, items) {
        let renderPackOtherTextField = false;
        items.map(item => {
            if(item.code === 'SPK99') {
                renderPackOtherTextField = true;
            }
        });
        this.setState({renderPackOtherTextField});
    }
    ...
    render() {
        ...
        const packTypeOptions = this.props.packTypes && this.props.packTypes.toJS();
                    ...
                    <div>
                        ...
                        <div>
                            <Field
                                ...
                                getOptionValue={(option) => option.code}
                                onChange={this.packTypeMultiSelectOnChange}
                            />
                            {
                                this.state.renderPackOtherTextField?
                                    <Field
                                        name="packOther"
                                        label="* 其他盛裝方式"
                                        placeholder="其他"
...
function mapStateToProps(state) {
    return {
        counties: state.enterprise.sludgeTreatment.get("COUNTIES"),
        townships: state.enterprise.sludgeTreatment.get("TOWNSHIPS"),
        items: state.enterprise.sludgeTreatment.get('ITEMS'),
        packTypes: state.enterprise.sludgeTreatment.get('PACKTYPES'),
