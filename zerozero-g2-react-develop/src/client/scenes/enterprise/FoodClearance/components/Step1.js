...
class ReservationStep1 extends Component {
    ...
    _labelSelectBoxRender(data) {
        ...
        for (let i = 0; i < jsData.length; i++) {
            let item = {};
            item.value = jsData[i].id;
            item.label = jsData[i].name;
    ...
    convertUnitsOptions(data, valueKey, labelKey) {
        let optionData = [{value: -1, label: "選擇"}];
        if (!data) {
            return optionData;
        }
        ...
        for (let i = 0; i < data.length; i++) {
            let item = {};
            item.value = data[i][valueKey];
            item.label = data[i][labelKey];
    ...
    render() {
        ...
        const unitOptions = this.props.units && this.props.units.toJS();
                    ...
                    <div>
                        ...
                        <div className="multi_col">
                            ...
                            <Field name="unit"
                                   component={LabelSelectBox}
                                   label="* 報廢模式"
                                   options={this.convertUnitsOptions(unitOptions, 'code', 'title')}
                            />
                        </div>
                        <Field
                            name="comment"
...
function mapStateToProps(state) {
    return {
        ...
        items: state.enterprise.foodClearance.get('ITEMS'),
        units: state.enterprise.foodClearance.get('UNITS'),
