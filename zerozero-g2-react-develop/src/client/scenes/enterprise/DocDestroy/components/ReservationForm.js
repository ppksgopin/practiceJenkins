...
const Container = styled.div`
	...
	.step{
		...
		&.step1{
			>div.pageImage{
				background:url(${awsUrl("epFormStep1_doc_20190101.png")}) no-repeat center center;
...
class ReservationForm extends Component {
	...
	joinLineButton() {
        gtag('event','conversion',{'send_to':'AW-851066983/GRJNCLyh53sQ54DplQM'});
    }

    

    toSignUp() {
        gtag('event','conversion',{'send_to':'AW-851066983/TA1hCJm2-XsQ54DplQM'});
    }
    ...
    submit(values) {
    	...
    	new Promise((resolve, reject) => {
    		resolve(this.props.submitDocDestroyForm(values));
		})
			.then((response) => {
                this.nextPage();
            })
...
function mapStateToProps(state) {
    return {
        counties: state.enterprise.docDestroy.get("COUNTIES"),
        townships: state.enterprise.docDestroy.get("TOWNSHIPS"),
    }
}
