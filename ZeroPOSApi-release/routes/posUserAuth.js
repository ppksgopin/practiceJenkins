 ...
 *   post:
 *     tags: [系統]
 *     summary: 查詢前台POS使用者的場區授權
 *     description: 使用 POST 方法，透過 user_id 查詢前台POS使用者的場區授權
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
...
router.post('/', withDatabase, async (req, res) => {
    ...
    try {
        // Step 1: Find pos_user_ouid using user_id
        const userQuery = `
            SELECT ouid 
            FROM pos_user 
            WHERE user_id = $1 
        ...
        const posUserOuid = userResult.rows[0].ouid;

        // Step 2: Find storeshop_ouid using pos_user_ouid
        const authQuery = `
            SELECT storeshop_ouid 
            FROM storeshop_pos_user_auth 
            WHERE pos_user_ouid = $1`;

        logWithDebug(`Executing SQL: ${authQuery} with params: [${posUserOuid}]`);
        const authResult = await client.query(authQuery, [posUserOuid]);
        logWithDebug(`Query Result: ${JSON.stringify(authResult.rows)}`);

        if (authResult.rows.length === 0) {
            logWithWarn(`No auth found for posUserOuid: ${posUserOuid}`);
